...
if (defined('WORLD_MAX')) {
    $MIN_X = -WORLD_MAX; $MAX_X = WORLD_MAX;
    $MIN_Y = -WORLD_MAX; $MAX_Y = WORLD_MAX;
} else {
    $res = mysqli_query($database->dblink, "SELECT MIN(x) AS mnx, MAX(x) AS mxx, MIN(y) AS mny, MAX(y) AS mxy FROM `$VDATA`");
    if ($res && $r = mysqli_fetch_assoc($res)) {
        $MIN_X = (int)$r['mnx']; $MAX_X = (int)$r['mxx'];
        $MIN_Y = (int)$r['mny']; $MAX_Y = (int)$r['mxy'];
    } else {
        $MIN_X = $MIN_Y = -100; $MAX_X = $MAX_Y = 100;
    }
}
